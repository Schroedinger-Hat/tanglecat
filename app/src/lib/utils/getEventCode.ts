export function getEventCodeFromEnvironment(): string | null {
  const envCode = process.env.NEXT_PUBLIC_TANGLECAT_EVENT_CODE
  return envCode || null
}

export function getEventCode(): string | null {
  // Check environment variable first
  const envCode = getEventCodeFromEnvironment()
  if (envCode) {
    return envCode
  }
