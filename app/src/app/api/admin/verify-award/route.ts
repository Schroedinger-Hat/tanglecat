...
export async function GET(request: Request) {
  try {
    ...
    if (!supervisor) {
      return NextResponse.json({ message: "Not authorized to verify this award" }, { status: 403 })
    }
    ...
    if (!player.eventCodes?.some((code: { _ref: string }) => code._ref === eventId)) {
      return NextResponse.json({ message: "Player not registered for this event" }, { status: 403 })
    }
    ...
    const totalChallengePoints =
      player.completedChallenges?.reduce(
        (sum: number, challenge: { points: number }) => sum + (challenge.points || 0),
        0,
      ) || 0

    // Calculate points already used for awards
    const usedAwardPoints =
      player.receivedAwards?.reduce(
        (sum: number, award: { points: number }) => sum + (award.points || 0),
        0,
      ) || 0
    ...
    if (availablePoints < award.points) {
      return NextResponse.json({ message: "Insufficient points for this award" }, { status: 400 })
    }
    ...
    return NextResponse.json({ message: "Failed to verify award" }, { status: 500 })
  }
}